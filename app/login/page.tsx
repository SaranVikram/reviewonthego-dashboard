...
const LoginPage: React.FC = () => {
  ...
  return (
    <div className="w-[450px] rounded-lg border border-secondary py-[30px] px-[100px] bg-white">
      <div
        className="w-[170px] mt-5 mb-6 mx-auto block"
        style={{ height: "fit-content" }}
      >
        <img src="/images/brand/brand-01.svg" alt="" title="" height="35" />
      </div>
      <h1 className="text-center text-black font-normal text-lg !leading-9 mb-5">
        Hello, who’s this?
      </h1>
      <form className="flex flex-col mx-auto my-0">
        <div className="relative block m-0 rounded-[4px] !mb-4 w-full">
          <div className="absolute left-0 right-auto top-0 bottom-0 p-[1px]">
            <div
              ref={containerRef}
              className="relative cursor-pointer z-1 w-[46px] h-full flex justify-center items-center"
              tabIndex={0}
              title="India (भारत): +91"
              onClick={toggleList}
            >
            ...
            {showList && (
              <FlagList ulRef={ulRef} handleCountryClick={handleCountryClick} />
            )}
          </div>
          <input
            type="tel"
            autoComplete="off"
            className="py-4 !m-0 pr-[6px] pl-[52px] relative w-full"
            value={phoneNumber}
        ...
        <div
          style={{ display: "flex" }}
          className="w-full mx-auto my-0 justify-between !mb-4"
        >
          <div style={{ display: "flex", alignItems: "center" }}>
            <input
              ...
              className="block font-normal leading-normal text-black bg-white bg-clip-padding appearance-none rounded p-[10px] !w-[50px]"
              maxLength={1}
              value={otp[0]}
              onChange={(e) => handleInputChange(e, 0, otpInput2)}
              onKeyDown={(e) => handleKeyDown(e, 0, undefined, otpInput2)}
            />
          </div>
          <div style={{ display: "flex", alignItems: "center" }}>
            <input
              ...
              maxLength={1}
              value={otp[1]}
              onChange={(e) => handleInputChange(e, 1, otpInput3)}
              onKeyDown={(e) => handleKeyDown(e, 1, otpInput1, otpInput3)}
            />
          </div>
          <div style={{ display: "flex", alignItems: "center" }}>
            <input
              ...
              maxLength={1}
              value={otp[2]}
              onChange={(e) => handleInputChange(e, 2, otpInput4)}
              onKeyDown={(e) => handleKeyDown(e, 2, otpInput2, otpInput4)}
            />
          </div>
          <div style={{ display: "flex", alignItems: "center" }}>
            <input
              ...
              onKeyDown={(e) => handleKeyDown(e, 3, otpInput3, undefined)}
            />
          </div>
        </div>
        {phoneRequest ? (
          <button
            type="button"
            className="cursor-pointer text-[#fff] bg-primary border-primary text-sm py-2 px-4"
